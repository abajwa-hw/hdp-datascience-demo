...
def _hadoop2_jars(hadoop_home):
    jars = _jars_from_dirs([os.path.join(hadoop_home, d) for d in (
        '',
        'client',
    )])
    jar_root_cldr = os.path.join(hadoop_home, 'share', 'hadoop')
    cldr_jars = _jars_from_dirs([os.path.join(jar_root_cldr, d) for d in (
        'hdfs',
        'common',
